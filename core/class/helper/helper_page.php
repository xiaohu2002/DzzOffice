...
"'.$ajaxtarget.' title="下一页">»</a></li>' : '').
			...
			$multipage = $multipage ? '<div class="text-center p-2"><ul class="pagination '.($classname?$classname:'').'">'.$multipage.($shownum && !$simple ? '<li class="disable"><a class="page-link" title="共有'.$num.'条记录">'.$num.'</a></li>' : '').'</ul></div>' : '';
		}
		$maxpage = $realpages;
		return $multipage;
	}
