...
if($_GET['action'] == 'checkupgrade') {
	...
	if($_G['member']['adminid'] == 1) {
		...
		if($list){
			...
			$html.= '<strong class="me-auto">'.lang('upgrade_notice_title').'</strong>';
			$html.=' <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close" onclick="jQuery(\'#systemNotice\').hide();setcookie(\'upgradenotice\',1,3600);"></button>';
			$html.=' </div>';
			$html.='<div class="toast-body" style="padding:0;max-height:500px;overflow-y:auto">';
